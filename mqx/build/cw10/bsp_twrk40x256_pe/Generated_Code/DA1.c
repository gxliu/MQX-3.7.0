...
**     Component : DAC_LDD
**     Version   : Component 01.005, Driver 01.01, CPU db: 3.00.001
**     Compiler  : CodeWarrior ARM C Compiler
**     Date/Time : 2011-03-28, 23:52, # CodeGen: 32
**     Abstract  :
...
**         GetEventStatus       - LDD_TEventMask DA1_GetEventStatus(LDD_TDeviceData *DeviceDataPtr);
...
/*
** ===================================================================
**     Method      :  DA1_GetEventStatus (component DAC_LDD)
**
**     Description :
**         This method returns state of interrupt requests and clears
**         pending interrupt flags. Return value has the same format as
**         EventMask parameter of SetEventMask method. Can be used for
**         polling mode without using events. This method is available
**         only if DMA service is not enabled.
**     Parameters  :
**         NAME            - DESCRIPTION
**       * DeviceDataPtr   - Device data structure
**                           pointer returned by <Init> method.
**     Returns     :
**         ---             - Actual state of interrupt requests.
** ===================================================================
*/
LDD_TEventMask DA1_GetEventStatus(LDD_TDeviceData *DeviceDataPtr)
{
  uint8_t State;
  ...
  _int_disable();
  State = DAC_PDD_GetInterruptFlags(DAC0_BASE_PTR); /* Save pending interrupts state */
...
**                           - ERR_DISABLED - Component or device is
**                           disabled.
**                           - ERR_NOTAVAIL - HW trigger is selected or
**                           buffer is disabled.
** ===================================================================
*/
LDD_TError DA1_ForceSwTrigger(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  if (DAC_PDD_GetBufferEnabled(DAC0_BASE_PTR) == PDD_DISABLE) { /* Is buffer enabled? */
    return ERR_NOTAVAIL;               /* No, return ERR_NOTAVIAL */
  }
  if (DAC_PDD_GetTriggerSource(DAC0_BASE_PTR) == DAC_PDD_HW_TRIGGER) { /* Is SW trigger enabled? */
    return ERR_NOTAVAIL;               /* No, return ERR_NOTAVIAL */
  }
...
LDD_TError DA1_SetBufferSize(LDD_TDeviceData *DeviceDataPtr, uint8_t Size)
{
  ...
  /* Buffer size value test - this test can be disabled by setting the "Ignore range checking"
     property to the "yes" value in the "Configuration inspector" */
  if(Size > DA1_BUFFER_MAX_SIZE) {     /* Is required buffer size in possible range? */
    return ERR_PARAM_SIZE;             /* No, return ERR_PARAM_SIZE */
  }
  DAC_PDD_SetBufferSize(DAC0_BASE_PTR,Size);
  return ERR_OK;
}
